...
def plot_images(images):
    ...
    images = images / 2 + 0.5  # reverse of normalize
...
def save_images(images, path, **kwargs):
    ...
    ndarr = grid.permute(1, 2, 0).to('cpu').numpy() * 255
...
image = images[0]
diff = Diffusion(img_size=img_size, device="cpu")
t = torch.Tensor([50, 100, 150, 200, 300, 600, 700, 999]).long()
noised_image, sigma = diff.noise_images(image, t)
save_images(noised_image.add(1).mul(0.5), "noise.jpg")
...
# y = x.new_tensor([1] * x.shape[0]).long()  # this is for conditional generation
print(net(x, t).shape)
...
diffusion = Diffusion(img_size=64, device=device, noise_steps=25)
